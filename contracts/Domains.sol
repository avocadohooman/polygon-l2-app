...
contract Domains is ERC721URIStorage {
	...
	*/
	function register(string calldata _newDomainName) public payable{
		...
		string memory _domainName = string(abi.encodePacked(_newDomainName, ".", tld));
		string memory _finalSvg = string(abi.encodePacked(svgPartOne, _domainName, svgPartTwo));
		uint256 newRecordId = _tokenIds.current();
		uint256 length = StringUtils.strlen(_newDomainName);
		string memory strLen = Strings.toString(length);
		
		console.log("Registering %s.%s on the contract with tokenID %d", _newDomainName, tld, newRecordId);
